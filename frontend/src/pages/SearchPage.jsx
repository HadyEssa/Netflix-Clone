...
const handleSearch = async (e) => {
  ...
  try {
    ...
    const res = await axios.get(`/api/v1/search/${activeTab}/${encodedSearchTerm}`);
    
    // Check if we have results
    if (res.data && (res.data.results || res.data.content)) {
      setResults(res.data.results || res.data.content);
    } else {
      setResults([]);
